...
var helpers = {
  ...
  destroy: function (sessionId, options, callback) {
    ...
    var attempts = options.retries || 5;
    var intervalTime = 3000; // 3초 간격
    var intervalId;

    var attemptToRemove = function () {
        fs.remove(sessionPath, function (removeError) {
            if (!removeError) {
                // 성공적으로 삭제했으므로 인터벌을 정리합니다.
                clearInterval(intervalId);
                if (callback) callback(null);
            } else if (attempts <= 1) {
                // 마지막 시도 이후에도 실패하면 인터벌을 정리하고 오류를 반환합니다.
                clearInterval(intervalId);
                if (callback) callback(removeError);
            }
            attempts--;
        });
    };

    // 첫 번째 삭제 시도를 즉시 시작합니다.
    attemptToRemove();

    // 실패할 경우 인터벌로 반복합니다.
    intervalId = setInterval(attemptToRemove, intervalTime);
},
